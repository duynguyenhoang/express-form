...
function Field(property, label) {
  ...
  this.__alwaysValidateErrors = false;
  this.__skipValidateArrayItem = false;
  this.ignoreErrors = function () {
    return !this.__required && !this.__alwaysValidateErrors;
  };
  ...
  this.array = function(skipValidateArrayItem) {
    if (skipValidateArrayItem) {
      this.__skipValidateArrayItem = true;
    }
    isArray = true;
    return this;
  };
  ...
  this.run = function (source, form, options, cb) {
    ...
    function runStack(foo, cb) {
      ...
      async.eachSeries(stack, function(proc, cb) {
        ...
        if (result.error) {
          ...
          if (!utils.hasValue(value) && self.ignoreErrors()) return cb(null);
    ...
    if (isArray) {
      ...
      if (this.__skipValidateArrayItem) {
...
Field.prototype.array = function (skipValidateArrayItem) {
  return this.array(skipValidateArrayItem);
};

Field.prototype.arrLength = function (from, to, messages) {
  return this.add(function (value) {
    if (value.length < from) {
      return { error: (messages && messages[0]) || "%s is too short" };
    }
    if (value.length > to) {
      return { error: (messages && messages[1]) || "%s is too long" };
    }
    return { valid: true };
  });
};
...
Field.prototype.alwaysValidateErrors = function () {
  this.__alwaysValidateErrors = true;
  return this.add(function() {
    return { valid: true };
  });
};

Field.prototype.arraySize = function (size, message) {
  this.array(true);
  return this.add(function(value) {
    if (value.length > size) {
      return { error: message || "%s array size is too big, limited at " + size };
    }
    return { valid: true };
  });
};
